...
namespace Cast {
	inline static TEHitResult Raycast(TAGE::Entity* caller, const glm::vec3& from, const glm::vec3& to, bool ignoreSelf = true, TERayDrawType draw = TERayDrawType::FOR_FRAME, float draw_time = 0.0f) { TECast::CurrentCaller = caller; TEHitResult res = TECast::Raycast(from, to, ignoreSelf, draw, draw_time); TECast::CurrentCaller = nullptr; return res; }
	inline void CreateRigidBodyWithCollider(TAGE::Entity* actor, float mass = 1.0f) { actor->AddComponent<TColliderComponent>(); GetWorld().GetPhysicsSystem().CreateRigidBody(actor, mass); }
}
